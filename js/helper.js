...
function changeDate(gmtTimeString) {
  ...
  return `${gmtDate.getUTCHours()}:${gmtDate.getUTCMinutes()}:${gmtDate.getUTCSeconds()} ${gmtDate.getUTCDate()}/${
      ...
      .forEach((e) => {
        ...
        div.innerHTML = `
			<button class="-link d-block col-12 text-left rounded-4 px-20 py-15 js-search-option">
			  <div class="d-flex">
				<div class="">
				<img src="${e.cover}" alt="image" class="js-lazy loaded" data-ll-status="loaded" style="width: 50px; height: 50px; object-fit:cover;">
				</div>
				<div class="ml-10">
...
function getSearchValueAndNavigate(locations) {
  ...
  btnSubmit.addEventListener("click", () => {
    if (searchInput.value || searchInput.value != "") {
      locationParam = `location=${searchInput.value.trim()}`;
    ...
    if (datePickerInput.value || datePickerInput.value != "") {
      ...
      durationParam = `duration=${calcDuration(startDate, endDate)}`;
    ...
    if (urlParams.length == 0) {
      ...
      url = domain + "/tours?" + urlParams.join("&");
